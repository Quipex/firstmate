...
The lock claim itself is an atomic symlink, and MSYS's default `ln -s` deep-copies a directory instead of linking it, which turns every acquire into a failure and its wait into a hang, so `fm_lock_try_create` requests `winsymlinks:nativestrict` and fails loudly when real links are unavailable.
...
- Live acquisition inside a real Pi session under Herdr: `FM_STATE_OVERRIDE="$(mktemp -d)" bin/fm-lock.sh` printed `lock acquired: harness pid 23040`, where 23040 is the live `node.exe` running the pi bundle, and a follow-up `bin/fm-lock.sh status` printed `lock: held by live harness pid 23040`.
